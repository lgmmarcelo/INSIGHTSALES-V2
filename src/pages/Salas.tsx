...
interface SalaStats {
  ...
  empreendimentos: Record<string, EmpreendimentoStats>;
}
...
export default function Salas() {
  ...
  const groupedData = useMemo(() => {
    ...
    rawSales.forEach(sale => {
      ...
      const cleanEmp = EMPREENDIMENTO_MAP[rawEmp] || rawEmp;
      ...
      if (!map[salaName].empreendimentos[cleanEmp]) {
        map[salaName].empreendimentos[cleanEmp] = { brutas: 0, cancelamentos: 0 };
      }

      map[salaName].brutasTotais++;
      map[salaName].empreendimentos[cleanEmp].brutas++;

      if (includesCancellation) {
        map[salaName].cancelamentosTotais++;
        map[salaName].empreendimentos[cleanEmp].cancelamentos++;
      }
    });
      ...
      {groupedData.length === 0 && !loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 items-start pb-10">
          {groupedData.map((sala) => {
             ...
             // Sort empreendimentos by number of sales
             const emps = Object.entries(sala.empreendimentos)
                        .map(([nome, d]) => ({nome, ...d}))
             ...
             return (
               <div key={sala.nome} className="bg-white rounded-xl shadow border border-slate-200 overflow-hidden flex flex-col hover:shadow-md transition-shadow">
                  ...
                  {/* Body (List of Empreendimentos) */}
                  <div className="flex-1 p-0 flex flex-col bg-white">
                    <table className="w-full text-left border-collapse">
                      <thead className="bg-slate-50">
                        <tr>
                          <th className="px-4 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200">Empreendimento</th>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {emps.map(emp => {
                          const taxaEmp = emp.brutas > 0 ? ((emp.cancelamentos / emp.brutas) * 100).toFixed(1) : '0.0';
                          const isAlta = parseFloat(taxaEmp) > 25;
                          const liquidas = emp.brutas - emp.cancelamentos;
                          const taxaLiq = emp.brutas > 0 ? ((liquidas / emp.brutas) * 100).toFixed(1) : '0.0';
                          ...
                          return (
                            <tr key={emp.nome} className="hover:bg-slate-50 transition-colors">
                               ...
                               <td className="px-2 py-3 text-sm font-black text-slate-600 text-center">{emp.brutas}</td>
                               <td className="px-2 py-3 text-sm font-black text-slate-500 text-center">{emp.cancelamentos}</td>
